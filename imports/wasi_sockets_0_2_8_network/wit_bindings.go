...
const (
	// Unknown error
	ErrorCode_Unknown uint8 = 0
	// Access denied.
	//
	// POSIX equivalent: EACCES, EPERM
	ErrorCode_AccessDenied uint8 = 1
	// The operation is not supported.
	//
	// POSIX equivalent: EOPNOTSUPP
	ErrorCode_NotSupported uint8 = 2
	// One of the arguments is invalid.
	//
	// POSIX equivalent: EINVAL
	ErrorCode_InvalidArgument uint8 = 3
	// Not enough memory to complete the operation.
	//
	// POSIX equivalent: ENOMEM, ENOBUFS, EAI_MEMORY
	ErrorCode_OutOfMemory uint8 = 4
	// The operation timed out before it could finish completely.
	ErrorCode_Timeout uint8 = 5
	// This operation is incompatible with another asynchronous operation that is already in progress.
	//
	// POSIX equivalent: EALREADY
	ErrorCode_ConcurrencyConflict uint8 = 6
	// Trying to finish an asynchronous operation that:
	// - has not been started yet, or:
	// - was already finished by a previous `finish-*` call.
	//
	// Note: this is scheduled to be removed when `future`s are natively supported.
	ErrorCode_NotInProgress uint8 = 7
	// The operation has been aborted because it could not be completed immediately.
	//
	// Note: this is scheduled to be removed when `future`s are natively supported.
	ErrorCode_WouldBlock uint8 = 8
	// The operation is not valid in the socket's current state.
	ErrorCode_InvalidState uint8 = 9
	// A new socket resource could not be created because of a system limit.
	ErrorCode_NewSocketLimit uint8 = 10
	// A bind operation failed because the provided address is not an address that the `network` can bind to.
	ErrorCode_AddressNotBindable uint8 = 11
	// A bind operation failed because the provided address is already in use or because there are no ephemeral ports available.
	ErrorCode_AddressInUse uint8 = 12
	// The remote address is not reachable
	ErrorCode_RemoteUnreachable uint8 = 13
	// The TCP connection was forcefully rejected
	ErrorCode_ConnectionRefused uint8 = 14
	// The TCP connection was reset.
	ErrorCode_ConnectionReset uint8 = 15
	// A TCP connection was aborted.
	ErrorCode_ConnectionAborted uint8 = 16
	// The size of a datagram sent to a UDP socket exceeded the maximum
	// supported size.
	ErrorCode_DatagramTooLarge uint8 = 17
	// Name does not exist or has no suitable associated IP addresses.
	ErrorCode_NameUnresolvable uint8 = 18
	// A temporary failure in name resolution occurred.
	ErrorCode_TemporaryResolverFailure uint8 = 19
	// A permanent failure in name resolution occurred.
	ErrorCode_PermanentResolverFailure uint8 = 20
)
...
const (
	...
	IpAddressFamily_Ipv4 uint8 = 0
	// Similar to `AF_INET6` in POSIX.
	IpAddressFamily_Ipv6 uint8 = 1
)
...
const (
	IpAddress_Ipv4 uint8 = 0
	IpAddress_Ipv6 uint8 = 1
)
...
const (
	IpSocketAddress_Ipv4 uint8 = 0
	IpSocketAddress_Ipv6 uint8 = 1
)
...
func (self IpSocketAddress) Ipv4() Ipv4SocketAddress {
	if self.tag != IpSocketAddress_Ipv4 {
		panic("tag mismatch")
	}
...
func (self IpSocketAddress) Ipv6() Ipv6SocketAddress {
	if self.tag != IpSocketAddress_Ipv6 {
		panic("tag mismatch")
	}
...
func MakeIpSocketAddressIpv4(value Ipv4SocketAddress) IpSocketAddress {
	return IpSocketAddress{IpSocketAddress_Ipv4, value}
}
func MakeIpSocketAddressIpv6(value Ipv6SocketAddress) IpSocketAddress {
	return IpSocketAddress{IpSocketAddress_Ipv6, value}
}
